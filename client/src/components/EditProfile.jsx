...
const editProfileHandler = async () => {
    ...
    if (!input.bio || !input.gender) {
        toast.error("Bio and Gender are required fields!");
        return;
    }
    ...
    // Ensure profile photo is valid and append
    if (input.profilePhoto) {
        if (input.profilePhoto instanceof File) {
            formData.append("profilePhoto", input.profilePhoto);
        } else {
            toast.error("Invalid file format for profile photo.");
            return;
        }
    }
    ...
    try {
        ...
        if (res.data.success) {
            ...
            dispatch(setAuthUser(updatedUserData));  // Update user data in global state
            navigate(`/profile/${user?._id}`);  // Navigate to updated profile
